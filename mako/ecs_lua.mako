...
---@field ${name} ${Name}Component
    %endif
% endfor
%if extensions['Entity'] != None:
    % for funName ,value in extensions['Entity'].items():
<%
        name, ret_type = funName.split(':')
    %>\
---@field ${name} fun(self:Entity,${','.join(value)}):${ret_type}
    % endfor
%endif
...
---@field is${Name}Entity bool
    %endif
%endfor
%if extensions['Pool'] != None:
    % for funName ,value in extensions['Pool'].items():
<%
        name, ret_type = funName.split(':')
    %>\
---@field ${name} fun(self:Pool,${','.join(value)}):${ret_type}
    % endfor
%endif
