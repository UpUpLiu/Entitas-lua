...
function M:create_entity()
    ...
    if not entity then
        ...
        entity.on_component_added:add(self._comp_added, self)
        entity.on_component_removed:add(self._comp_removed, self)
        entity.on_component_replaced:add(self._comp_replaced, self)
    end
...
function M:_comp_replaced(entity, old_comp, new_comp)
    for _, group in pairs(self._groups) do
        if group._matcher:match_one(new_comp) then
            group:update_entity(entity, old_comp, new_comp)
        end
    end
end

function M:destroy_all_entity()
